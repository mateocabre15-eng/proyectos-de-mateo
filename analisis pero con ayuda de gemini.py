...
class Portafolio():
    ...
    def graficar_frontera(self, num_simulaciones=5000, rf=0.02):
        resultados = []
        sigma = self.matriz_covarianzas.values
        n = sigma.shape[0]
        ...
        # 1. Hacemos las simulaciones igual que antes
        for _ in range(num_simulaciones):
            ...
            resultados.append((riesgo, retorno, sharpe))
        ...
        # ==========================================
        # EL FILTRO MÁGICO PARA LIMPIAR LA FRONTERA
        # ==========================================
        
        retorno_min_var = self.weights_optimos @ self.retornos_individuales
        
        # A. Descartamos la mitad ineficiente de abajo
        simulaciones_eficientes = [sim for sim in self.simulaciones if sim[1] >= retorno_min_var]
        
        # B. Ordenamos los puntos de izquierda a derecha (por riesgo)
        simulaciones_eficientes.sort(key=lambda x: x[0])
        
        frontera_limpia = []
        max_retorno_visto = -np.inf
        
        # C. Solo guardamos los puntos que formen el "borde" superior
        for sim in simulaciones_eficientes:
            riesgo, retorno, sharpe = sim
            # Si este punto me da más retorno que todo lo anterior, lo sumo a la curva
            if retorno > max_retorno_visto:
                frontera_limpia.append(sim)
                max_retorno_visto = retorno
                
        # ==========================================

        # Separamos los datos pero usando solo la 'frontera_limpia'
        x = [sim[0] for sim in frontera_limpia]
        y = [sim[1] for sim in frontera_limpia]
        sharpe_ratios = [sim[2] for sim in frontera_limpia]
        ...
        fig = pyl.scatter(
            ...
            labels={'x': 'Riesgo (Volatilidad)', 'y': 'Retorno Esperado', 'color': 'Ratio de Sharpe'}
        )

        # Tu estrella roja de Mínima Varianza
        fig.add_scatter(
        ...
        # La estrella dorada del Máximo Sharpe Ratio
        fig.add_scatter(
            x=[opt_riesgo], 
            y=[opt_retorno], 
            mode='markers', 
            marker=dict(color='gold', size=15, symbol='star', line=dict(width=1, color='black')),
            name='Máximo Sharpe (Óptimo)'
        )

        fig.update_layout(
            title="Frontera Eficiente de Markowitz (Curva Óptima)",
